/*
 * LinuxVstPlugin.cpp - LMMS VST Support Layer (RemotePlugin client)
 *
...
class LinuxVstPlugin;

LinuxVstPlugin * __plugin = NULL;





class LinuxVstPlugin : public RemotePluginClient
{
public:
	LinuxVstPlugin( key_t _shm_in, key_t _shm_out );
	virtual ~LinuxVstPlugin();
...
LinuxVstPlugin::LinuxVstPlugin( key_t _shm_in, key_t _shm_out ) :
...

{
	...
	{
		perror( "LinuxVstPlugin.cpp::ftok" );
	}
	else
	{	// connect to shared memory segment
		if( ( m_shmID = shmget( key, 0, 0 ) ) == -1 )
		{
			perror( "LinuxVstPlugin.cpp::shmget" );
		}
		else
		{	// attach segment
			...
			{
				perror( "LinuxVstPlugin.cpp::shmat" );
			}
		}
	}
#else
	...
	{
		fprintf(stderr, "LinuxVstPlugin.cpp: "
...
bool LinuxVstPlugin::processMessage( const message & _m )
{
	switch( _m.id )
	{
...
void LinuxVstPlugin::init( const std::string & _plugin_file )
{
...
void LinuxVstPlugin::initEditor()
{
...
bool LinuxVstPlugin::load( const std::string & _plugin_file )
{
	if( ( m_libInst = LoadLibrary( _plugin_file.c_str() ) ) == NULL )
	{
		// give VstPlugin class a chance to start 32 bit version of LinuxVstPlugin
...
void LinuxVstPlugin::process( const sampleFrame * _in, sampleFrame * _out )
{
...
void LinuxVstPlugin::processMidiEvent( const MidiEvent& event, const f_cnt_t offset )
{
	VstMidiEvent vme;
...
const char * LinuxVstPlugin::pluginName()
{
	static char buf[32];
	buf[0] = 0;
...
const char * LinuxVstPlugin::pluginVendorString()
{
	static char buf[64];
	buf[0] = 0;
...
const char * LinuxVstPlugin::pluginProductString()
{
	static char buf[64];
	buf[0] = 0;
...
const char * LinuxVstPlugin::programName()
{
	static char buf[24];
...
void LinuxVstPlugin::sendCurrentProgramName()
{
	char presName[64];
...
void LinuxVstPlugin::getParameterDump()
{
	lock();
...
void LinuxVstPlugin::setParameterDump( const message & _m )
{
	lock();
...
void LinuxVstPlugin::getParameterProperties( const int _idx )
{
...
void LinuxVstPlugin::saveChunkToFile( const std::string & _file )
{
...
void LinuxVstPlugin::setProgram( int program )
{
...
void LinuxVstPlugin::rotateProgram( int offset )
{
	if( isInitialized() == false )
	{
		return;
...
void LinuxVstPlugin::getProgramNames()
{
...
void LinuxVstPlugin::savePreset( const std::string & _file )
{
...
void LinuxVstPlugin::loadPresetFile( const std::string & _file )
{
	void * chunk = NULL;
...
void LinuxVstPlugin::loadChunkFromFile( const std::string & _file, int _len )
{
	char * buf = NULL;
...
void LinuxVstPlugin::updateInOutCount()
{
	delete[] m_inputs;
	delete[] m_outputs;
 ...
 */
intptr_t LinuxVstPlugin::hostCallback( AEffect * _effect, int32_t _opcode,
					int32_t _index, intptr_t _value,
						void * _ptr, float _opt )
{
...
DWORD WINAPI LinuxVstPlugin::processingThread( LPVOID _param )
{
	LinuxVstPlugin * _this = static_cast<LinuxVstPlugin *>( _param );
...
DWORD WINAPI LinuxVstPlugin::guiEventLoop( LPVOID _param )
{
	LinuxVstPlugin * _this = static_cast<LinuxVstPlugin *>( _param );
...
int main( int _argc, char * * _argv )
{
	...
	__plugin = new LinuxVstPlugin( atoi( _argv[1] ), atoi( _argv[2] ) );

	if( __plugin->isInitialized() )
	{
		__GuiThreadID = GetCurrentThreadId();
		if( CreateThread( NULL, 0, LinuxVstPlugin::processingThread,
		...
		LinuxVstPlugin::guiEventLoop( __plugin );
	}
